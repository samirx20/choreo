...
### Decision 126: Cross-Scene Magic Move Transitions, Kinetic Action Moves, & 3D Mockup MCP Parity
* **The Problem**:
  1. **"PowerPoint" Stagnation**: Multi-scene videos generated by AI models looked like static slide decks (elements appeared one by one, sat motionless, and hard-cut to the next scene).
  2. **Root Architectural Deficiencies**:
     - **Zero Cross-Scene Interpolation**: On scene boundaries, `videoExporter.ts` and `PixiStage` executed an instantaneous 100% hard cut, tearing down the previous scene and mounting the next. Even when sequential scenes shared element IDs (Apple Keynote Magic Move pattern), no interpolation occurred.
     - **Crippled Action Keyframes in MCP**: `apply_animation` in `mcp.js` lacked `params` (`toX`, `toY`, `toScale`, `fromX`, `fromY`), `from` offsets, and `fillMode` ("forwards"). When an agent wanted to move an element from center to top or zoom into a card, the tool rejected or dropped those coordinates, freezing elements as statues.
     - **3D Mockup Absence in MCP**: `place_element` and `update_element` lacked the `mockup3d` type, preventing agents from staging 3D device showcases (iPhone, MacBook, credit card) with camera presets and screen slot texture projections.
* **The Solution**:
  1. **PixiStage Cross-Scene Magic Move Engine (`src/engine/pixi/PixiStage.ts`)**:
     - Implemented `renderTransition(fromScreen, toScreen, rawProgress, transition)` and `lerpColorHex`.
     - Analyzes element pairs sharing the same `id` across `fromScreen` and `toScreen`.
     - Interpolates shared elements across `(x, y, width, height, scaleX, scaleY, rotation, opacity, fontSize)` using analytical spring and easing curves (`snappy`, `smooth`, `bouncy`, `ease-out`, etc.).
     - Coordinates outgoing elements with directional exits (`fade`, `slideLeft`, `slideRight`, `slideUp`, `slideDown`) and incoming elements with smooth reveals.
  2. **Unified Timeline Frame Stepping in Exporter (`src/engine/export/videoExporter.ts`)**:
     - Created `renderTimelineFrame(pixiStage, timeline, itemIndex, localTime, renderState)`.
     - Detects cross-scene transitions (`transition.type !== "cut" && transitionDuration > 0 && localTime < transitionDuration`) and invokes `renderTransition(prevItem.screen, activeItem.screen, progress, transition)`.
     - Shared across native Tauri base64 FFmpeg streaming, browser MediaRecorder, and GIF export pipelines.
  3. **Full MCP Tool Suite Parity (3D Mockups & Kinetic Action Moves)**:
     - Added `mockup3d` layer type to `place_element` and `update_element` schemas in `mcp.js` (supporting `modelType: "iphone" | "macbook" | "card"`, `cameraPreset`, `position3D`, `rotation3D`, `scale3D`, `cameraFov`, `cameraPosition`, `cameraTarget`, and `screenSlot: { sourceId }`).
     - Enriched `apply_animation` schema in `mcp.js` to support `params` (`toX`, `toY`, `fromX`, `fromY`, `toScale`, `fromScale`, `toRotate`, `toOpacity`, `toBlur`, `toColor`, `distance`, `angle`), `from` offsets, and `fillMode: "forwards" | "backwards" | "both" | "none"` (default `"forwards"` for action clips).
     - Synchronized all 42 MCP tool JSON schemas in `C:\Users\Sam\.gemini\antigravity\mcp\motion-studio/`.
     - Updated `instructions.md` with Recipe 4 (3-Beat Kinetic Lifecycle with Action Moves) and Recipe 5 (Cross-Scene Magic Move).
* **Verification**:
  - Added unit test in `src/test/multi_scene_export_and_popover.test.ts` verifying `renderTransition` execution during video export for scenes with `transition.type = "magicMove"`.
...
### Decision 129: Full GUI & MCP Feature and Perceptual Parity, Multi-Clip Timelines & Recursive Layer Resolution

* **Context & Motivation**:
  - Motion graphics generated by AI agents lacked dynamism and fine-grained choreography because agent tools were historically constrained to a single entrance slot (`enter`). Sequential calls to `apply_animation` blew away prior clips or failed when elements were grouped, framed, or masked.
  - Mid-scene action transforms (moving or resizing an element midway through a scene to clear stage for another element) were impossible because clips defaulted to non-persistent states without `fillMode: 'forwards'`, causing elements to snap back to origin.
  - Agents suffered an extreme "perceptual blackout": `get_storyboard_state` returned only raw layer IDs and preset names, omitting actual coordinates, dimensions, typography, styles, counter configurations, and clip timing intervals, forcing agents to choreograph blindly.
  - `mcp.js` tool operations relied on shallow `screen.layers.find(...)` and `splice(...)`, causing silent failures or errors when operating on nested children inside groups, frames, compound cards, and masks.

* **The Solution**:
  1. **Multi-Clip Animation Timelines & Lifecycle Roles**:
     - Extended `AnimationClipInputSchema`, `ApplyAnimationInputSchema`, and `PlaceElementInputSchema` in `src/types/agentTools.ts`.
     - Supports the complete visual lifecycle: `in` entrance $\to$ mid-scene `action` moves/rescales/rotations $\to$ `emphasis` pulses $\to$ `out` exit.
     - Supports both `mode: 'append'` (chains onto existing clips at element end time or explicit delay) and `mode: 'replace'` (replaces existing clips).
     - Mid-scene `action` clips default to `fillMode: 'forwards'` so transformed coordinates and scales persist across subsequent scene beats.
     - Supports batch multi-clip provisioning directly inside `place_element` and `apply_animation` via `animations: [...]`.
  2. **Deep Recursive Layer Resolution & Splicing**:
     - Implemented `findLayerInTree(layers, targetId)` and `findLayerInDoc(doc, targetId)` with depth-first traversal returning `{ layer, screen, parentList, index }`.
     - Replaced all 18 shallow `screen.layers.find` / `findIndex` calls across `mcp.js`: `place_element`, `update_element`, `delete_element`, `duplicate_element`, `reorder_element`, `ungroup_elements`, `align_elements`, `apply_animation`, `remove_animation`, `update_animation_clip`, `stagger_elements`, `link_elements`, `unlink_elements`, `split_text`, `split_shape`, `split_line`, `separate_stroke_fill`, and `join_lines_into_shape`.
     - Splicing and hierarchy-modifying tools operate directly on `res.parentList.splice(res.index, ...)` so nested elements inside groups, frames, cards, and masks are seamlessly manipulated without breaking parent containers.
  3. **High-Signal Spatial & Perceptual Telemetry**:
     - Rewrote `get_storyboard_state` and `get_contact_sheet`: returns complete spatial `bounds: { x, y, width, height, rotation, opacity, zIndex }`, typography copy and styling (`content, fontSize, fontWeight, color, textAlign`), visual styling (`fillColor, strokeColor, strokeWeight, borderRadius, shadowColor, padding`), counter configurations (`start, end, duration, prefix, suffix`), reactive bindings, nested `children`, and complete clip timelines (`id, role, preset, start, duration, end, easing, fillMode, direction`).
  4. **Horizontal & Vertical Spacing Distribution**:
     - Added `distribute_horizontal` and `distribute_vertical` math to `align_elements` in `mcp.js` with common aliases (`distribute_h`, `distribute_v`, `distribute_x`, `distribute_y`).
  5. **MCP Schema Synchronization**:
     - Synchronized all 42 MCP tool JSON schemas in `C:\Users\Sam\.gemini\antigravity\mcp\motion-studio/`.
     - Updated `instructions.md` Recipe 2 to showcase multi-clip animation choreography.
